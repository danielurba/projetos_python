...
"bem" : "#$# O Grande Investidor - VERSION BETA 1.0 #$#",
"feito" : "#$# By DANIEL #$#"
}
...
def quest1pt():
    conta.append(score1)
    print()
    print(f"#$# Nome da Conta: {user.title()} #$#\n#$# Saldo em Conta: {conta[0]},00 R$ #$#")
    print()
    true = True
    while true:
...
def faturporce():
    ...
    if quest1 != None:
        ...
        qp1result = qp1 * int(conta[0])
        qp1resultf = int(conta[0]) - int(qp1result)
...
def login_score_value():
    with open("Score.txt", "r") as log:
        lines = log.readlines()
        for line in lines:
            if line.split()[0] == user:
                global score1
                score1 = line.strip().split()[2]
        ...
        with open("Score.txt", "r") as log:
            ...
            for line in lines:
                ...
                with open("Score.txt", "a") as f:
                    score = login + " " + password + ' ' + str(int(100000))
                    f.write(str(score + '\n'))
                    print('#$# Criando Login... #$#\n')
                    sleep(2)
...
def writescore():
    with open("Score.txt", "r") as writscor:
        lines = writscor.readlines()
    with open("Score.txt", "w") as writscor2:
        for line in lines:
            if line.strip('\n') != user + ' ' + senha + ' ' + str(int(score1)):
                writscor2.write(line)
    with open("Score.txt", "a") as writscor3:
        score = user + " " + senha + ' ' + str(int(conta[0]))
        writscor3.write(str(score))

def readscores():
    with open("Score.txt", "r") as myscore:
        variav = myscore.read()
        variav = variav.split()
        res = map(int, (variav[2::3]))
        user = variav[::3]
        score = list(res)
...
def play():
    while True:
        print(f'~~~~#$# MENU DO JOGO = Usuario logado: {user.title()} #$#~~~~\n')
        play = input("#$# DIGITE #$#\n#$# S para INICIAR o Jogo #$#\n#$# V para SCORES #$#\n#$# X para SAIR da conta: ")
        print()
        if play == "s":
            login_score_value()
        elif play == 'v':
...
def users():
    while True:
        try:
            with open("Score.txt", "r") as myscore2:
                variav = myscore2.read()
                variav = variav.split()
                userf = variav[::3]
                senhaf = variav[1::3]
                users = zip(userf, senhaf)
                userfinal = dict(users)
        except FileNotFoundError:
            with open("Score.txt", "w") as t:
                pass
            with open("Score.txt", "r") as myscore3:
                variav = myscore3.read()
                variav = variav.split()
                userf = variav[::3]
                senhaf = variav[1::3]
                users = zip(userf, senhaf)
                userfinal = dict(users)
        print('~~~~#$# MENU #$#~~~~\n')
        primary = input('#$# DIGITE #$#\n#$# A para ACESSAR conta #$#\n#$# C para CRIAR uma conta #$#\n#$# V para Scores #$#\n#$# X para sair Jogo: ')
        print()
        if primary == 'a' or primary == 'A':
            ...
            senha = input('Senha: ')
            print()
            for chave, valor in userfinal.items():
                if user == chave and senha == valor:
                    print('#$# Logando... #$#\n')
                    sleep(2)
                ...
                print('#$# Usuario e/ou senha inválido(s) #$#\n')
